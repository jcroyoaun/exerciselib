  ...
  return (
    <div className="h-full flex">
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-neutral-900">
        <div className="bg-neutral-800 border-b-4 border-white/30 px-6 py-4">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setShowFilters(!showFilters)}
              className="p-2 bg-neutral-700 border-2 border-white/40 hover:bg-neutral-600 transition-colors"
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-2.5 bg-neutral-700 border-2 border-white/40 text-white placeholder-white/30 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-white/60 focus:border-white/60"
              />
              {searchTerm && (
                <button
                  onClick={() => {
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-neutral-600"
                >
            ...
            <button
              ...
              className="px-4 py-2.5 bg-white text-neutral-950 border-4 border-neutral-300 font-mono font-bold hover:bg-neutral-200 transition-colors text-sm uppercase tracking-wider"
            >
              SEARCH
            </button>
          </div>
          ...
          {hasActiveFilters && (
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              {typeFilter.map(type => (
                <span key={type} className="inline-flex items-center gap-1 px-2 py-1 bg-neutral-700 border-2 border-white/50 text-white text-xs font-mono">
                  {type}
                  <button onClick={() => toggleType(type)} className="hover:bg-neutral-600 ml-1">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
              {muscleFilters.map(id => {
                ...
                return muscle ? (
                  <span key={id} className="inline-flex items-center gap-1 px-2 py-1 bg-neutral-700 border-2 border-green-500 text-green-400 text-xs font-mono">
                    {muscle.name}
                    <button onClick={() => toggleMuscle(id)} className="hover:bg-neutral-600 ml-1">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                ) : null;
              ...
              {patternFilters.map(name => (
                <span key={name} className="inline-flex items-center gap-1 px-2 py-1 bg-neutral-700 border-2 border-cyan-500 text-cyan-300 text-xs font-mono">
                  {name}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-neutral-900">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              <div className="p-4 bg-neutral-800 border-4 border-white/40 mb-4">
                <Search className="w-8 h-8 text-white/50" />
              </div>
              <h3 className="text-lg font-mono font-bold text-white mb-2">// NO RESULTS FOUND</h3>
            ...
            <>
              <div className="space-y-2 mb-6">
                <div className="grid grid-cols-12 gap-4 px-4 py-2 bg-neutral-800 border-b-2 border-white/40">
                  <div className="col-span-4 text-white font-mono text-xs uppercase tracking-wider font-bold">Exercise</div>
                  <div className="col-span-2 text-white font-mono text-xs uppercase tracking-wider font-bold">Type</div>
                  <div className="col-span-3 text-white font-mono text-xs uppercase tracking-wider font-bold">Pattern</div>
                  <div className="col-span-2 text-white font-mono text-xs uppercase tracking-wider font-bold">Muscles</div>
                  <div className="col-span-1 text-white font-mono text-xs uppercase tracking-wider text-right font-bold">Actions</div>
                </div>
              ...
              {metadata && metadata.last_page > 1 && (
                <div className="flex items-center justify-center gap-3">
                  <button
                    ...
                    disabled={page === 1}
                    className="px-4 py-2 bg-neutral-800 border-2 border-white/40 text-white font-mono text-sm hover:bg-neutral-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                  >
                    &lt; PREV
                  </button>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 p-4 bg-green-500 text-neutral-950 rounded-none shadow-lg hover:bg-green-600 transition-all hover:scale-110 border-4 border-green-600"
